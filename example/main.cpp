...
#define CASE(enum) \
    ...
    throw "Invalid Heuristic: " + heuristic_string;
}
...
void read_overlapping_instance(xml_node<> *node) {
    ...
    unsigned width = stoi(get_attribute(node, "width", size));
    unsigned height = stoi(get_attribute(node, "height", size));

    unsigned N = stoi(get_attribute(node, "N", "3"));
...
// #define CASE(x) \
//     if (symmetry_name == #x) return Symmetry::x
...
//     // Well
//     if (symmetry_name == "\\") return Symmetry::backslash;
//     throw "Invalid Symmetry: " + symmetry_name;
...
//  */
// optional<unordered_set<string>> read_subset_names(xml_node<> *root_node,
//                                                   const string &subset) {
...
//     while (subset_node && get_attribute(subset_node, "name") != subset) {
...
//         subset_names.insert(get_attribute(node, "name"));
//     }
//     return subset_names;
// }
...
//                                               const string &subset,
//                                               unsigned size) {
//     optional<unordered_set<string>> subset_names =
...
//             continue;
//         }
//         Symmetry symmetry = to_symmetry(get_attribute(node, "symmetry",
//         "X")); double weight = stod(get_attribute(node, "weight", "1.0"));
//         const string image_path = current_dir + "/" + name + ".png";
//         optional<Array2D<Color>> image = read_image(image_path);

//         if (image == nullopt) {
//             vector<Array2D<Color>> images;
//             for (unsigned i = 0; i < nb_of_possible_orientations(symmetry);
//                  i++) {
...
//  */
// vector<tuple<string, unsigned, string, unsigned>> read_neighbors(
//     xml_node<> *root_node) {
...
//         string left = get_attribute(node, "left");
...
//             left_orientation = stoi(left.substr(left_delimiter,
//             string::npos));
//         }

//         string right = get_attribute(node, "right");
...
//             right_orientation =
//                 stoi(right.substr(right_delimiter, string::npos));
//         }
//         neighbors.push_back(
...
//     string name = get_attribute(node, "name");
//     string subset = get_attribute(node, "subset", "tiles");
//     bool periodic_output = (get_attribute(node, "periodic", "False") ==
//     "True"); unsigned width = stoi(get_attribute(node, "width", "48"));
//     unsigned height = stoi(get_attribute(node, "height", "48"));

//     cout << name << " " << subset << " started!" << endl;

//     ifstream config_file("samples/" + name + ".xml");
...
//     unsigned size = stoi(get_attribute(data_root_node, "size"));
...
//     for (unsigned test = 0; test < 10; test++) {
//         int seed = get_random_seed();
//         TilingWFC<Color> wfc(tiles, neighbors_ids, height, width,
//                              {periodic_output}, seed);

//         // For the summer tileset, place water on the borders, and land in
//         the
//         // middle
//         if (name == "Summer") {
...
//         optional<Array2D<Color>> success = wfc.run();
//         if (success.has_value()) {
//             write_image_png("results/" + name + "_" + subset + ".png",
//                             *success);
