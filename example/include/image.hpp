 ...
 */
std::optional<Array2D<Color>> read_image(
    ...
    for (unsigned i = 0; i < (unsigned)height; i++) {
        for (unsigned j = 0; j < (unsigned)width; j++) {
            ...
            m.data[i * width + j] = {data[index], data[index + 1],
                                     data[index + 2]};
        }
    }
    free(data);
...
void write_image_png(const std::string& file_path,
                     const Array2D<Color>& m) noexcept {
    stbi_write_png(file_path.c_str(), m.width, m.height, 3,
                   (const unsigned char*)m.data.data(), 0);
}
