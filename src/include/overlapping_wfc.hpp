...
struct OverlappingWFCOptions {
    ...
    unsigned
        symmetry;  // The number of symmetries (the order is defined in wfc).
...
class OverlappingWFC {
   private:
     ...
     * parameters.
     */
    OverlappingWFC(const Array2D<T> &input,
                   const OverlappingWFCOptions &options, const int &seed,
     ...
     * The lowest middle pattern is used as a floor (and ceiling when the input
     * is toric) and is placed at the lowest possible pattern position in the
     * output image, on all its width. The pattern cannot be used at any other
     * place in the output image.
     */
     ...
     */
    static unsigned get_ground_pattern_id(
        const Array2D<T> &input, const std::vector<Array2D<T>> &patterns,
        const OverlappingWFCOptions &options) noexcept {
        // Get the pattern.
    ...
    /**
     * Return the list of patterns, as well as their probabilities of
     * apparition.
     */
    static std::pair<std::vector<Array2D<T>>, std::vector<double>> get_patterns(
        const Array2D<T> &input,
        const OverlappingWFCOptions &options) noexcept {
        ...
        for (unsigned i = 0; i < max_i; i++) {
            for (unsigned j = 0; j < max_j; j++) {
                ...
                symmetries[0].data =
                    input
                        .get_sub_array(i, j, options.pattern_size,
                ...
                // The number of symmetries in the option class define which
                // symetries will be used.
                for (unsigned k = 0; k < options.symmetry; k++) {
                    auto res = patterns_id.insert(
                        std::make_pair(symmetries[k], patterns.size()));

                    // If the pattern already exist, we just have to increase
                    // its number of appearance.
                    if (!res.second) {
    ...
    static bool agrees(const Array2D<T> &pattern1, const Array2D<T> &pattern2,
        ...
        // Iterate on every pixel contained in the intersection of the two
        // pattern.
        for (unsigned y = ymin; y < ymax; y++) {
            for (unsigned x = xmin; x < xmax; x++) {
                // Check if the color is the same in the two patterns in that
                // pixel.
     ...
     * If agrees(pattern1, pattern2, dy, dx), then
     * compatible[pattern1][direction] contains pattern2, where direction is the
     * direction defined by (dy, dx) (see direction.hpp).
     */
    ...
    generate_compatible(const std::vector<Array2D<T>> &patterns) noexcept {
        ...
        for (unsigned pattern1 = 0; pattern1 < patterns.size(); pattern1++) {
            for (unsigned direction = 0; direction < 4; direction++) {
                for (unsigned pattern2 = 0; pattern2 < patterns.size();
                     pattern2++) {
                    if (agrees(patterns[pattern1], patterns[pattern2],
                               directions_y[direction],
     ...
     */
    Array2D<T> to_image(
        const Array2D<unsigned> &output_patterns) const noexcept {
        ...
        if (options.periodic_output) {
            for (unsigned y = 0; y < options.get_wave_height(); y++) {
                for (unsigned x = 0; x < options.get_wave_width(); x++) {
                    output.get(y, x) =
                        patterns[output_patterns.get(y, x)].get(0, 0);
                }
            }
        } else {
            for (unsigned y = 0; y < options.get_wave_height(); y++) {
            ...
            for (unsigned y = 0; y < options.get_wave_height(); y++) {
                const Array2D<T> &pattern = patterns[output_patterns.get(
                    y, options.get_wave_width() - 1)];
                for (unsigned dx = 1; dx < options.pattern_size; dx++) {
                    output.get(y, options.get_wave_width() - 1 + dx) =
                        pattern.get(0, dx);
                }
            }
            for (unsigned x = 0; x < options.get_wave_width(); x++) {
                const Array2D<T> &pattern = patterns[output_patterns.get(
                    options.get_wave_height() - 1, x)];
     ...
     * pattern_id needs to be a valid pattern id, and i and j needs to be in the
     * wave range
     */
   ...
   public:
     ...
     */
    OverlappingWFC(const Array2D<T> &input,
                   const OverlappingWFCOptions &options, int seed) noexcept
     ...
     */
    bool set_pattern(const Array2D<T> &pattern, unsigned i,
                     unsigned j) noexcept {
        auto pattern_id = get_pattern_id(pattern);

        if (pattern_id == std::nullopt || i >= options.get_wave_height() ||
            j >= options.get_wave_width()) {
            return false;
        }
