...
class Propagator {
   ...
   private:
     ...
     * opposite direction of direction without being in contradiction with
     * pattern placed in (y,x). If wave.get(y, x, pattern) is set to false, then
   ...
   public:
    ...
    void add_to_propagator(unsigned y, unsigned x, unsigned pattern) noexcept {
        // All the direction are set to 0, since the pattern cannot be set in
        // (y,x).
