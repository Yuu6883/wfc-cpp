...
struct Tile {
     ...
     * Actions 0, 1, 2, and 3 are 0°, 90°, 180°, and 270° anticlockwise
    ...
    Tile(Array2D<T> data, Symmetry symmetry, double weight) noexcept
        : data(generate_oriented(data, symmetry)),
          symmetry(symmetry),
          weight(weight) {}
};
...
class TilingWFC {
    ...
    generate_propagator(
        ...
        for (auto neighbor : neighbors) {
            ...
            auto add = [&](unsigned action, unsigned direction) {
                ...
                dense_propagator[oriented_tile_id1][direction]
                                [oriented_tile_id2] = true;
                direction = get_opposite_direction(direction);
                dense_propagator[oriented_tile_id2][direction]
                                [oriented_tile_id1] = true;
            };
                ...
                for (unsigned y = 0; y < size; y++) {
                    for (unsigned x = 0; x < size; x++) {
                        tiling.get(i * size + y, j * size + x) =
                            tiles[oriented_tile.first]
                                .data[oriented_tile.second]
                                .get(y, x);
                    }
                }
            }
   ...
   public:
     ...
     */
    bool set_tile(unsigned tile_id, unsigned orientation, unsigned i,
                  unsigned j) noexcept {
        if (tile_id >= oriented_tile_ids.size() ||
            orientation >= oriented_tile_ids[tile_id].size() || i >= height ||
            j >= width) {
            return false;
        }
