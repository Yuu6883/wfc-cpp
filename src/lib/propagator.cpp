...
void Propagator::propagate(Wave &wave) noexcept {
    ...
    while (propagating.size() != 0) {
        ...
        for (unsigned direction = 0; direction < 4; direction++) {
            ...
            // For every pattern that could be placed in that cell without being
            // in contradiction with pattern1
            for (auto it = patterns.begin(), it_end = patterns.end();
                 it < it_end; ++it) {
                // We decrease the number of compatible patterns in the opposite
                // direction If the pattern was discarded from the wave, the
                // element is still negative, which is not a problem
                std::array<int, 4> &value = compatible.get(y2, x2, *it);
                value[direction]--;

                // If the element was set to 0 with this operation, we need to
                // remove the pattern from the wave, and propagate the
                // information
